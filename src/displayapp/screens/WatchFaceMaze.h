  ...
  namespace Applications {
    namespace Screens {
      ...
      struct MazeTile {
        ...
        bool getUp() const        {return map & UPMASK;}
        bool getLeft() const      {return map & LEFTMASK;}
        bool getFlagEmpty() const {return map & FLAGEMPTYMASK;}
        bool getFlagGen() const   {return map & FLAGGENMASK;}
      };
      ...
      // Maybe faster too? Probably not by much if it is.
      class MazeRNG {
      public:
      ...
      private:
        ...
        static constexpr int8_t MAX_START_ANGLE = 45;  // degrees off from straight vertical a particle can be going when spawned (must be <90)
      ...
      private:
        // Functions related to refreshing, for better separation of processes
        void HandleMazeRefresh();
        void HandleConfetti();  // okay it's not very well separated but at least the confetti part is separate
        ...
        void SeedMaze();
        void PutTimeDate();  // Puts time onto the watchface. Part of SeedMaze, Do not call directly.
        ...
        void GenerateMaze();
        void GeneratePath(int x, int y); // Generates a single path starting at the x,y coords. Part of GenerateMaze, do not call directly.
        ...
        // Buffers for use during printing. There's two it flips between because if there was only one, it would start
        //  being overwritten before the DMA finishes, and it'd corrupt parts of the display.
        // activeBuffer is, well, the currently active one. Switch with swapActiveBuffer();
        lv_color_t buf1[480];
        lv_color_t buf2[480];
        lv_color_t *activeBuffer = buf1;
        void swapActiveBuffer() {activeBuffer = (activeBuffer==buf1) ? buf2 : buf1;}
        ...
        // After a code is inputted the code is cleared, so if making a code smaller than the max size take care that it doesn't overlap
        //  with any other code.
        ...
        constexpr static uint8_t foxCode[7] = {0,1,0,3,2,1,2};  // a healthy secret in a curious game :3
