...
// ########## Definitions of gaps between and sizes of objects
...
constexpr uint8_t bar1_x = cells_x - 2 * bargap;
constexpr uint8_t bar2_x = cells_x - bargap;
...
constexpr lv_color_t COLOR_BG = LV_COLOR_BLACK;
...
constexpr lv_color_t COLOR_THEME_NORM[] = {COLOR_LIGHTBLUE,
                                           COLOR_ORANGE,
                                           COLOR_DARKGRAY,
                                           COLOR_BLUE,
                                           COLOR_DARKGRAY,
...
constexpr lv_color_t COLOR_THEME_AOD[] = {LV_COLOR_WHITE,
                                          LV_COLOR_RED,
                                          LV_COLOR_RED,
                                          LV_COLOR_BLUE,
                                          LV_COLOR_RED,
...
void WatchFaceStarTrek::drawWatchFace(bool visible) {
  ...
  batteryPlugOrNotificationStatus = label(visible, COLOR_ICONS, listItem4[1], LV_ALIGN_CENTER, -gap, 0, Symbols::plug);
...
void WatchFaceStarTrek::Refresh() {
  ...
  if (powerPresent.IsUpdated()) {
    lv_label_set_text_static(batteryPlugOrNotificationStatus, BatteryIcon::GetPlugIcon(powerPresent.Get()));
    // fontawesome and material icons have slightly different y alignment.
    lv_obj_align(batteryPlugOrNotificationStatus, listItem4[1], LV_ALIGN_CENTER, -gap, 0);
    // if charge off, possibly display status again
    notificationStatus.MarkUpdated();
  }
  // TODO figure out the logic here
  if (!powerPresent.Get()) {
    notificationStatus = settingsController.GetNotificationStatus();
    if (notificationStatus.IsUpdated()) {
      if (notificationStatus.Get() == Controllers::Settings::Notification::Off) {
        lv_label_set_text_static(batteryPlugOrNotificationStatus, Symbols::notificationsOff);
      } else if (notificationStatus.Get() == Controllers::Settings::Notification::Sleep) {
        lv_label_set_text_static(batteryPlugOrNotificationStatus, Symbols::sleep);
      } else {
        lv_label_set_text_static(batteryPlugOrNotificationStatus, "");
      }
      lv_obj_align(batteryPlugOrNotificationStatus, listItem4[1], LV_ALIGN_CENTER, -gap, 1);
    }
  }
...
void WatchFaceStarTrek::animateStartStep() {
  switch (animatorStartStage) {
    ...
    case 4:
      ...
      lv_obj_set_hidden(batteryPlugOrNotificationStatus, false);
      break;
    case 5:
...
void WatchFaceStarTrek::setVisible(bool visible) {
  ...
  lv_obj_set_hidden(batteryPlugOrNotificationStatus, !visible);
...
void WatchFaceStarTrek::setColorTheme(const lv_color_t* colors) {
  ...
  if (heartbeatRunning.Get()) {
    lv_obj_set_style_local_text_color(heartbeatIcon, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, colors[16]);
  } else {
...
void WatchFaceStarTrek::createMenu() {
  ...
  lblSetUseSystemFont = label(true, LV_COLOR_WHITE, btnSetUseSystemFont, LV_ALIGN_CENTER, 0, 0, label_sysfont, btnSetUseSystemFont);
  btnSetAnimate = button(true, 224, 50, btnSetUseSystemFont, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 8);
  lblSetAnimate = label(true, LV_COLOR_WHITE, btnSetAnimate, LV_ALIGN_CENTER, 0, 0, animateMenuButtonText(), btnSetAnimate);
  ...
  lblSetWeather = label(true, LV_COLOR_WHITE, btnSetWeather, LV_ALIGN_CENTER, 0, 0, label_weather, btnSetWeather);
  ...
  lblSetDisplaySeconds =
    label(true, LV_COLOR_WHITE, btnSetDisplaySeconds, LV_ALIGN_CENTER, 0, 0, label_displaySeconds, btnSetDisplaySeconds);
  btnClose = button(true, 80, 50, btnSetDisplaySeconds, LV_ALIGN_OUT_RIGHT_MID, 8);
  lblClose = label(true, LV_COLOR_WHITE, btnClose, LV_ALIGN_CENTER, 0, 0, "X", btnClose);
  showingMenu = true;
}
