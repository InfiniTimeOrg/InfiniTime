...
void Counter::Create() {
  ...
  static constexpr uint8_t hPadding = 5;
  static constexpr uint8_t vPadding = 16;
  const uint8_t width = lv_obj_get_width(number) + hPadding * 2;
  const uint8_t containerHeight = lv_obj_get_height(number) * 2 + vPadding * 2;
  static uint8_t btnHeight = containerHeight / 2;
  ...
  lv_obj_set_style_local_bg_opa(upBtn, LV_BTN_PART_MAIN, LV_STATE_DEFAULT, LV_OPA_0);
  ...
  lv_obj_set_style_local_bg_opa(downBtn, LV_BTN_PART_MAIN, LV_STATE_DEFAULT, LV_OPA_0);
  ...
  auto LineCreate = [&]() {
    ...
    lv_obj_set_style_local_line_color(line, LV_LINE_PART_MAIN, LV_STATE_DEFAULT, LV_COLOR_SILVER);
    lv_obj_set_style_local_line_opa(line, LV_LINE_PART_MAIN, LV_STATE_DEFAULT, LV_OPA_20);
    return line;
  };

  lowerLine = LineCreate();
  lv_obj_align(lowerLine, counterContainer, LV_ALIGN_CENTER, 0, -1);
}
